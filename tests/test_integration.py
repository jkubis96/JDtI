...
def test_difference_markers(jseq_object):
    jseq_object.calculate_difference_markers(min_exp=0, min_pct=0.25, n_proc=2, force=True)
...
def test_similarity(jseq_object):
    ...
    fig = jseq_object.similarity_plot(split_sets=True, set_info=True, cmap='seismic', width=6, height=4)
...
def test_spatial_similarity(jseq_object):
    fig = jseq_object.spatial_similarity(set_info=True, bandwidth=1, n_neighbors=5, min_dist=0.1,
                                         legend_split=4, point_size=20, spread=1.0,
...
def test_pca_umap(jseq_object):
    ...
    fig = jseq_object.UMAP_vis(names_slot='cell_names', set_sep=True, point_size=1, font_size=6,
                               legend_split_col=2, width=6, height=4, inc_num=True)
...
def test_statistics_and_volcano_and_scatter(jseq_object):
    stats = jseq_object.statistic(cells=None, sets='All', min_exp=0, min_pct=0.05, n_proc=2)
    assert not stats.empty
    fig = volcano_plot(stats)
    
    assert isinstance(fig, mpl.figure.Figure)
    
    stats_5 = stats.sort_values(['valid_group', 'esm', 'log(FC)'],
                                ascending=[True, False, False]).groupby('valid_group').head(5)
...
def test_compositions(jseq_object):
    jseq_object.data_composition(features_count=None, name_slot='cell_names', set_sep=True)
    fig = jseq_object.composition_pie(width=6, height=6, font_size=10,
                                cmap="tab20", legend_split_col=1,
                                offset_labels=0.5, legend_bbox=(1.15, 0.95))
    
    assert isinstance(fig, mpl.figure.Figure)

    fig = jseq_object.bar_composition(cmap='tab20b', width=2, height=6,
                                font_size=10, legend_split_col=1,
                                legend_bbox=(1.3, 1))
